   			...
   			<td>
   				<select class="easyui-combobox" style="width: 154px" id="typeId" name="typeId" editable="false" panelHeight="auto" >
				    ...
				    <c:forEach var="type" items="${typeCountList }">
				    	<option value="${type.typeid }"   <c:if test="${blog.typeid==type.typeid}">selected</c:if>     >${type.typename }</option>
				    </c:forEach>			
                </select>
   			</td>
   		</tr>
   			...
   			<td>
   				<select class="easyui-combobox" style="width: 154px" id="blogTypeId" name="blogTypeId" editable="false" panelHeight="auto" >
				    ...
				    <c:forEach var="blogType" items="${blogTypeCountList }">
				    	<option value="${blogType.blogtypeId }"   <c:if test="${blog.blogtypeid==blogType.blogtypeId}">selected</c:if>     >${blogType.typeName }</option>
				    </c:forEach>			
                </select>
   			</td>
   		</tr>
   		<tr>
   			...
   			<td>
				   <!-- <script id="editor" type="text/plain" style="width:100%;height:400px;"></script> -->
				    <textarea style="width:100%;height:500px;visibility:hidden;" id="content"   name="content" >${blog.content }</textarea>
   			</td>
   		</tr>
   		<tr>
   			<td>关键字：</td>
   			<td><input type="text/plain" id="keyWord" name="keyWord"  value="${blog.keyword }"  style="width: 980px;"/>&nbsp;(多个关键字中间用空格隔开)</td>
   		</tr>
...
 		var contentNoTag=dd.replace(/(^\s*)|(\s*$)/g,"");//dds为得到后的内容
		...
		if(title==null || title==''){
			...
			alert("请输入内容！");
			 ...
			 $.post("/manager/user/blog/save",{'blogid':blogid,'username':username,'title':title,'typeid':typeId,'blogtypeid':blogTypeId,'content':content,'summary':summary,'contentNoTag':contentNoTag,'keyword':keyWord},function(result){
				if(result.success){
					alert("博客修改成功！");
					resetValue();
				}else{
